(function(module) {
	...
	*/
	...
	var constants = Object.freeze({
			...
			name: 'OAuth',	// Something unique to your OAuth provider in lowercase, like "github", or "nodebb"
			oauth: {
				requestTokenURL: '',
				accessTokenURL: '',
			...
			oauth2: {
				authorizationURL: 'http://10.69.1.101/identitydemo/connect/authorize',
				tokenURL: 'http://10.69.1.101/identitydemo/connect/token',
				clientID: nconf.get('oauth:id'),	// don't change this line
				clientSecret: nconf.get('oauth:secret'),	// don't change this line
			},
			userRoute: 'http://10.69.1.101/identitydemo/connect/userinfo'	// This is the address to your app's "user profile" API endpoint (expects JSON)
			,scope:'openid,profile,address,roles,offline_access,multichannelmanagement'
	...
	OAuth.getStrategy = function(strategies, callback) {
		if (configOk) {
			...
			if (constants.type === 'oauth') {
				...
				opts.callbackURL = 'http://dra-ana-ded-lt:8080'+ '/auth/' + constants.name + '/callback'; ;
				
				winston.info('test error');


				
				passportOAuth.Strategy.prototype.userProfile = function(accessToken, done) {
					winston.error('user profile in');
			...
			strategies.push({
				...
				successUrl: '/auth/'+constants.name+'/callback'
			});
